...
def validate_numeric(new_value):
	# Permite cadena vacía (para borrar) y números con signo y decimales
	if new_value == "":
		return True
	pattern = r"^-?\d*(\.\d*)?$"
...
entry_a = tk.Entry(frame, validate='key', validatecommand=vcmd)
entry_a.grid(row=0, column=1)

tk.Label(frame, text="B:").grid(row=1, column=0)
entry_b = tk.Entry(frame, validate='key', validatecommand=vcmd)
